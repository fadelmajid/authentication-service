...
let obj = (objDB, db, rootpath) => {
  ...
  fn.verifySecretKey = async (secret_key) =>
    secret_key === config.secret_key ? true : false;
  ...
  fn.getValidAccessToken = async (access_token) => {
    ...
    let sql =
      "SELECT * FROM " +
      tbl.auth_token +
      " WHERE atoken_access = $1 AND atoken_status = $2 AND expired_date > $3 LIMIT 1";
  ...
  fn.getValidRefreshToken = async (refresh_token) => {
    ...
    let sql =
      "SELECT * FROM " +
      tbl.auth_token +
      " WHERE atoken_refresh = $1 AND atoken_status = $2 LIMIT 1";
  ...
  fn.getUserToken = async (id) => {
    // prepare sql query
    let sql =
      "SELECT * FROM " + tbl.auth_token + " WHERE atoken_id = $1 LIMIT 1";
    let row = await db.query(sql, [id]);
    return row.rows[0];
  };

  //get all auth token
  fn.getAllAuthToken = async (
    where = "",
    data = [],
  ...
  fn.getNewToken = async (atoken_id) => {
    ...
    let data = {
      ...
      expired_date: moment()
        .add(config.expire_token_hours, "hours")
  ...
  fn.setTokenInactive = async (device_id) => {
    ...
    let where = {
      cond: "atoken_device = $1 AND atoken_status = $2 ",
      bind: [device_id, "active"],
    };
    let data = {
  ...
  fn.getHistoryToken = async (data) => {
    let sql =
      "SELECT * FROM " +
      tbl.history_device +
      " WHERE atoken_device = $1 AND atoken_platform = $2 LIMIT 1 ";
  ...
  fn.insertHistoryToken = async (data) => {
    let sql =
      "INSERT INTO " +
      tbl.history_device +
      " (atoken_device,atoken_platform,created_date) VALUES ($1, $2, $3)";
    return await db.query(sql, [
      data.atoken_device,
      data.atoken_platform,
      data.created_date,
    ]);
  };
  // END HISTORY TOKEN
